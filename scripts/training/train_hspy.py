"""Train the autoencoder using PyTorch Lightning with .hspy file support."""
...
def load_hspy_data(file_path: Path, scan_step: int = 1, downsample: int = 1, 
                   downsample_mode: str = "bin", normalize: bool = True, 
                   sigma: float = 0.8) -> torch.Tensor:
    """
    Load .hspy file and convert to tensor format compatible with existing training.
    Based on reference code patterns but returns data in the format expected by train.py.
    """
    if not HAS_HYPERSPY:
    ...
    if downsample > 1:
        print(f"Downsampling by factor {downsample} using {downsample_mode}...")
        ...
        if downsample_mode == "bin":
            ...
            try:
                ...
                print("Warning: scipy not available, falling back to stride")
                data = data[:, :, ::downsample, ::downsample]
    
    print(f"Final data shape: {data.shape}")
    
    # Apply normalization like reference code
    if normalize:
    ...
    # Reshape like reference code: data.reshape(-1, 256, 256)
    ny, nx, qy, qx = data.shape
    data = data.reshape(-1, qy, qx)
    print(f"Reshaped for training: {data.shape}")
    
    # Convert to tensor and add channel dimension to match train.py format
    tensor = torch.from_numpy(data).float().unsqueeze(1)  # Add channel dimension: (N, 1, H, W)
...
def main():
    ...
    # .hspy specific arguments
    p.add_argument("--scan_step", type=int, default=1, help="Subsample scan positions by this factor")
    p.add_argument("--downsample", type=int, default=1, help="Downsample diffraction patterns by this factor")
    p.add_argument("--downsample_mode", choices=["bin", "stride", "gauss"], default="bin", help="Downsampling method")
    p.add_argument("--no_normalize", action="store_true", help="Skip log normalization")
    p.add_argument("--sigma", type=float, default=0.8, help="Gaussian sigma for gauss downsampling mode")
    ...
    # Load data from .hspy file instead of .pt file
    data = load_hspy_data(
        args.data,
        args.scan_step,
        args.downsample,
    ...
    # Detect input size from data (same as original train.py)
    ...
    # Split data into train/validation (80/20 split) - same as original train.py
    total_size = len(data)
